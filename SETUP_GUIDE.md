...
```javascript
// Whether to receive email notifications for new materials
NOTIFY_ON_NEW: true,

// Whether to receive email notifications for assignment deadlines
NOTIFY_DEADLINE: true,

// Timing of deadline notifications (days before)
DEADLINE_DAYS_TO_NOTIFY: [7, 3, 1, 0], // 7 days, 3 days, 1 day before, and today
```
...
- **Notification History**: Check the "Notification History" tab to see sent deadline alerts.
...
// 通知するタイミング（期限の何日前か）
DEADLINE_DAYS_TO_NOTIFY: [7, 3, 1, 0], // 7日前、3日前、1日前、当日
```
...
- **通知履歴**: スプレッドシートの「通知履歴」シートで送信済みの通知を確認
